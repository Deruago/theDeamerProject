...
namespace deamer::language::generator::definition::property::standard
{
	...
	public:
		 ...
		 *
		 *	\details Generators first need to be converted to their output/target, this function does this.
		 *
		...
		{
			return InSetOfGeneratorsAreDependenciesCovered_<sizeof...(Generators_), Generators_...>();
		}
		...
		static constexpr bool InSetOfGeneratorsAreDependenciesCovered_()
		{
			...
			{
				return InSetOfGeneratorsAreDependenciesCovered_<counter_ - 1, Generators_..., typename CurrentGenerator_::TargetPropertyDefinition_t>();
			}
			else
			{
				return AreRequiredDependenciesCoveredTillThisIsFound<CurrentGenerator_, Generators_...>();
			}
		}
		...
		{
			return InSetOfGeneratorsAreRequiredDependenciesCovered_<sizeof...(Generators_), Generators_...>();
		}

		/*!	\fn InSetOfGeneratorsAreRequiredDependenciesCovered_
		 *
		 *	\brief Converts a set of generators into their targets. And checks in the end if the dependencies are covered.
		 *
		 *	\tparam counter_ Used to track the remaining generators, that need to be converted.
		 *
		 *	\tparam CurrentGenerator_ Current generator.
		...
		static constexpr bool InSetOfGeneratorsAreRequiredDependenciesCovered_()
		{
			if constexpr (counter_ > 0)
			{
				return InSetOfGeneratorsAreRequiredDependenciesCovered_<counter_ - 1, Generators_..., typename CurrentGenerator_::TargetPropertyDefinition_t>();
			}
			else
			{
		...
		{
			return deamer::type::TypesAreAmongTypesTillSpecificTypeIsFound<PropertyType, DependenciesIncludingThis_...>::template value<Dependencies...>;
		}

		/*!	\fn AreRequiredDependenciesCovered
		 *
		 *	\brief Checks whether all required dependencies are covered for this generator. (The order doesn't matter)
		 *
		...
		{
			return deamer::type::TypesAreAmongTypes<AvailableDependencies_...>::template value<Dependencies...>;
		}
