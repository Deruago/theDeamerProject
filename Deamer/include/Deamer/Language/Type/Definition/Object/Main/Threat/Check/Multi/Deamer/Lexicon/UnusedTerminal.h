...
#ifndef DEAMER_LANGUAGE_DATASTRUCTURE_DEFINITION_OBJECT_MAIN_THREAT_CHECK_MULTI_DEAMER_LEXICON_UNUSEDTERMINAL_H
#define DEAMER_LANGUAGE_DATASTRUCTURE_DEFINITION_OBJECT_MAIN_THREAT_CHECK_MULTI_DEAMER_LEXICON_UNUSEDTERMINAL_H
...
namespace deamer::language::type::definition::object::main::threat::check::deamer::lexicon
{
	 ...
	 */
	class UnusedTerminal : public Multi<property::Type::Lexicon, property::Type::Grammar>,
						   public CheckOutput<threat::deamer::lexicon::UnusedTerminal>
	{
	protected:
		/*!	\fn AnalyzeDefinition
		 *
		 *	\brief This checks if some production rule references this terminal. If not
		 *	it is unused.
		 *
		 *	\note This threat assumes that there is a grammar present.
		 *
		 *	\note Deleted or "Crash" terminals are ignored as the are used by the lexer.
		 */
		std::vector<Threat*> AnalyzeDefinition(PropertyReference reference) override
		{
			...
			for (auto* const terminal : lexicon.Terminals)
			{
				if (terminal->Special == SpecialType::Delete ||
					terminal->Special == SpecialType::Crash)
				{
					continue;
				}
				...
				if (result.IsEmpty())
				{
					threats.push_back(new threat::deamer::lexicon::UnusedTerminal(terminal));
				}
			}
...
#endif // DEAMER_LANGUAGE_DATASTRUCTURE_DEFINITION_OBJECT_MAIN_THREAT_CHECK_MULTI_DEAMER_LEXICON_UNUSEDTERMINAL_H
